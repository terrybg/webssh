...
- 终端输入支持命令提示（通用+本机）；回车可自动收集到本机常用命令（可开关，上限≤1000，可配置收集行数）
- 注意：会话密码明文存储在 JSON 中，勿公网裸奔
- **安全提示**：开启自动收集时，终端里键入的内容（含密码、token 等敏感输入）可能被写入 `data/commands.json` 的本机会话命令（bySession）。输入密钥前请先在「本机常用命令」设置中关闭自动收集
