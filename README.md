...
开远程或文件时会校验登录态：过期则 **自动重新 login**，减少「会话已过期」打断。登录缓存按**会话 + 账号密码**校验：修改服务器用户名/密码后，新开远程/文件会 **强制用新凭据重登**，不会继续用旧连接。
...
| loginSsh / checkLogin / pwd | 登录、探活、shell 当前目录（共享连接按 ip:port:user，改密会重建） |
